...
Flusso di autenticazione
L'admin apre https://<worker>.workers.dev/auth nel browser.
Google chiede consenso per lo scope https://www.googleapis.com/auth/webmasters.readonly.
Google reindirizza su /callback?code=....
Il Worker scambia il code per access_token + refresh_token e li scrive in KV alla chiave tokens con il campo expires_at (timestamp ms).
Ad ogni chiamata API, getValidAccessToken controlla la scadenza e, se necessario, esegue un refresh automatico via refresh_token, riscrivendo i token in KV.
...
Tools MCP
Sono esposti 8 tools via tools/list. Tutti rispondono con content[].type = "text".
...
7. get_query_page_combinations
Estrae le combinazioni REALI Query × Pagina (dimensions: ["query", "page"]). A differenza di get_search_queries e get_top_pages — che aggregano su una sola dimensione — questo tool dice esattamente quale pagina si posiziona per quale query con le relative metriche per riga. È lo strumento di riferimento per individuare cannibalizzazioni SEO.

Parametro	Tipo	Required	Default	Descrizione
siteUrl	string	✅	—	URL proprietà.
startDate	string	✅	—	YYYY-MM-DD.
endDate	string	✅	—	YYYY-MM-DD.
limit	number	❌	1000	rowLimit GSC, cappato a 25000 (max API).
query	string	❌	—	Filtra a una specifica query lato API (operatore contains).
page	string	❌	—	Filtra a una specifica pagina/URL lato API (operatore contains).
minImpressions	number	❌	0	Filtro lato client: mostra solo le combinazioni con almeno N impressioni.
analyzeCannibalization	boolean	❌	false	Se true, raggruppa per query e mostra solo le query con 2+ pagine in competizione (ordinate per impressioni totali).
includeFreshData	boolean	❌	true	dataState: "all" vs "final".
In modalità standard restituisce l'elenco delle combinazioni ordinate per impressioni; con analyzeCannibalization=true produce direttamente il report delle query cannibalizzate, con le pagine concorrenti raggruppate sotto ciascuna query.
...
Mapping clienti → proprietà
Il Worker include direttamente nel sorgente (src/index.js:4) un CSV embedded ASSOCIAZIONI_CSV con ~787 righe che mappano Proprietà,Nome (lo stesso contenuto del file associazioni-gsc.csv versionato a fianco). Il file CSV non è letto a runtime: la fonte di verità è il template literal in index.js.

Funzioni interne:

parseAssociazioni() — costruisce la Map<nome, proprietà[]>.
cercaProprietaPerCliente(nome) — risolve un nome con match esatto, parziale, o multiplo.
getProprietaPrincipale(proprieta[]) — euristica per scegliere la proprietà "canonica" tra più varianti dello stesso cliente.
...
Crea un progetto su console.cloud.google.com.
...
Apri https://mcp-gsc-remote-moca.<account>.workers.dev/auth nel browser.
Completa il consenso Google (deve essere un account con accesso alle proprietà GSC).
Verifica con GET /auth/status che la risposta sia { "authenticated": true }.
Sviluppo locale
...
Quota API: GSC ha limiti di QPS / quota giornaliera per progetto Google Cloud — vedi la documentazione ufficiale.
Struttura del progetto
mcp-gsc-remoto/
├── src/
...
Wrangler — CLI di deploy.
Cloudflare KV — storage token OAuth.
Google Search Console API v3 — searchAnalytics/query, sites.
Google URL Inspection API v1.
Model Context Protocol — protocollo di comunicazione (@modelcontextprotocol/sdk ^1.18.2 in dependencies, ma il Worker implementa JSON-RPC manualmente senza usare la SDK).
Licenza
